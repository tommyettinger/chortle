...
import com.github.tommyettinger.chortle.ArabicData.CenterChar;
import com.github.tommyettinger.chortle.ArabicData.EndChar;
import com.github.tommyettinger.chortle.ArabicData.IndividualChar;
import com.github.tommyettinger.chortle.ArabicData.StartChar;

/**
 * Static utilities for right-to-left text.
 * <br>
...
public final class RtlUtils {
    ...
    /**
     * Checks if a char uses right-to-left directionality. Numbers are written left-to-right in the Arabic
     * script, but everything else is written right-to-left. All Hebrew letters and punctuation are written
     * right-to-left; there aren't Hebrew digits in Unicode. Almost everything else in Unicode uses left-to-right
     * directionality, including standard ASCII digits regardless of language context.
     *
     * @param c the char to evaluate
     * @return if the given char uses right-to-left directionality
     */
    public static boolean isRightToLeft(char c) {
        return Ranges.inRange(Ranges.HEBREW, c)
                || Ranges.inRange(Ranges.ARABIC_A, c)
                || Ranges.inRange(Ranges.ARABIC_B, c)
                || Ranges.inRange(Ranges.ARABIC_C, c)
                || Ranges.inRange(Ranges.ARABIC_SUPPLEMENT, c)
                || Ranges.inRange(Ranges.ARABIC_EXTENDED, c)
                || Ranges.inRange(Ranges.ARABIC_PRESENTATION_A, c)
                || Ranges.inRange(Ranges.ARABIC_PRESENTATION_B, c)
                ;
    }

    /**
     * Checks if a char is an Arabic letter or punctuation symbol that isn't a number. Numbers are written
     * left-to-right in the Arabic script, but everything else is written right-to-left.
     *
     * @param c the char to evaluate
     * @return if the given char is an Arabic-script glyph that isn't a number
     */
    public static boolean isArabicNonNumeric(char c) {
        return Ranges.inRange(Ranges.ARABIC_A, c)
                || Ranges.inRange(Ranges.ARABIC_B, c)
                || Ranges.inRange(Ranges.ARABIC_C, c)
                || Ranges.inRange(Ranges.ARABIC_SUPPLEMENT, c)
                || Ranges.inRange(Ranges.ARABIC_EXTENDED, c)
    ...
    public static boolean isComplexChar(Glyph glyph) {
        return glyph instanceof GlyphComplex;
    }
